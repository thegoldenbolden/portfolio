...
export async function POST(request: Request) {
 ...
 if (!body.track_name || !body.track_id) {
  return NextResponse.json({ message: "Missing track" }, { status: 400 });
 }
 ...
 try {
  ...
  if (error instanceof Prisma.PrismaClientKnownRequestError) {
   if (error.code === "P2002") {
    return NextResponse.json(
     { message: "This recommendation was previously made" },
     { status: 401 }
    );
   }
  }
  ...
  return NextResponse.json(
   { message: "There was an error submitting the recommendation" },
   { status: 500 }
  );
 }
}
